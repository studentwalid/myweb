...
if ( ! class_exists( 'Hustle_Admin_Page_Abstract' ) ) :
	...
	abstract class Hustle_Admin_Page_Abstract {
		...
		private function black_friday_notice() {
			if ( ! current_user_can( 'administrator' ) || ! is_main_site() || ! get_site_option( 'hustle_show_black_friday_2021' ) ) {
				return;
			}
			...
			if ( date_create( date_i18n( 'd-m-Y' ) ) >= date_create( date_i18n( '06-12-Y' ) ) ) {
				delete_site_option( 'hustle_show_black_friday_2021' );
				return;
			}

			wp_enqueue_script(
				'hustle-black-friday',
				Opt_In::$plugin_url . 'assets/js/black-friday.min.js',
				array( 'wp-i18n' ),
				Opt_In::VERSION,
				true
			);
			...
			$strings = array(
				'header'  => esc_html__( 'Black Friday Offer!', 'hustle' ),
				'message' => esc_html__( 'Get 11 Pro plugins on unlimited sites and much more with 50% OFF WPMU DEV Agency plan FOREVER', 'hustle' ),
				'notice'  => esc_html__( '*Only admin users can see this message', 'hustle' ),
				'link'    => 'https://wpmudev.com/black-friday/?coupon=BFP-2021&utm_source=hustle_' . ( Opt_In_Utils::_is_free() ? 'free' : 'pro' ) . '&utm_medium=referral&utm_campaign=bf2021',
			);

			wp_localize_script( 'hustle-black-friday', 'hustle_bf', $strings );

			echo '<div id="hustle-black-friday"></div>';
		}
